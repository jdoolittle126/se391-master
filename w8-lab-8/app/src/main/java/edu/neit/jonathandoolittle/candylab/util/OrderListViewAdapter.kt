...
class OrderListViewAdapter(private val applicationContext: Activity, private val orders: ArrayList<CandyOrder>) :
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        listItemDetails.text = String.format("ID: %d | %s ($%.2f) - %d", orders[position].id,
            orders[position].candyBarType.toString(),
            orders[position].totalPrice,
            orders[position].candyBarQuantity)
